function est = deepGLMlogitPoisson(X_train,y_train,est)
%DEEPGLMBINOMIAL Summary of this function goes here
...
[Loss_current,Accuracy_current] = deepGLMpredictLoss(Xval,yval,W_seq,beta,distr); % compute prediction loss
disp(['Initial loss: ',num2str(Loss_current)]);
PPS_DL(1) = Loss_current;
Accuracy_DL(1) = Accuracy_current;
...
rqmc = normrnd(0,1,S,d_theta+1); % using quasi MC random numbers      
for s=1:S
    U_normal = rqmc(s,:)';
    epsilon1=U_normal(1);
...
while ~stop 
    ...
    if (verbose && ~mod(iter,verbose))
        ...
        if(strcmp(distr,'binomial'))
            ...
            disp(['Current MSE: ',num2str(Accuracy_current)]);
        end
    end  
...
%         shrinkage_l2 = length(w_tilde_index)/mean_w_tilde;
    ...
    [Loss_current,Accuracy_current] = deepGLMpredictLoss(Xval,yval,W_seq,beta,distr); % compute prediction loss
    PPS_DL(iter) = Loss_current;
    Accuracy_DL(iter) = Accuracy_current;
...
% Showing that training phase has completed
disp('---------- Training Completed! ----------')
disp(['Number of iteration:',num2str(iter)]);
disp(['PPS best: ',num2str(min(PPS_DL))]);
if (strcmp(distr,'poisson'))
    disp(['MSE best: ',num2str(min(Accuracy_DL))]);
    est.out.mse = Accuracy_DL;
else
    disp(['Accuracy best: ',num2str(max(Accuracy_DL))]);
    est.out.accuracy = Accuracy_DL;
end
