function est = deepGLMTrainGUI(X_train,y_train,est,handles)
...
global stopFlag lbGlobal LassoGlobal
...
idxPatience = 0;       % Index of number of consequent non-decreasing iterations
                       % for early stopping
n_iter = epoch*num1Epoch; % Maximum iteration
% disp('---------- Training Phase ----------')
plot2handle = get(handles.axes_pps, 'UserData');
while ~stop 
    iter = iter+1;
    if(stopFlag)
        break;
    end
    % Calculate eslasped time  txtTime
    TimeNow = datetime('now');
    CPU = datestr(TimeNow-est.startTime,'HH:MM:SS');
    set(handles.txtTime,'string',CPU);
    set(handles.txtIter,'string',[num2str(iter),'/',num2str(n_iter)]);
    
    if(~mod(iter,num1Epoch))
        idxEpoch = idxEpoch + 1;
        set(handles.txtEpoch,'string',[num2str(idxEpoch),'/',num2str(epoch)]);
    end
    drawnow
...
%     X = minibatch(:,2:end);
    ...
    if(lbFlag)
        lb(iter) = mean(lb_iter)/batchsize;
        lb_current = lb(iter);
    end
    %----------------------------------------------------------------------
    ...
    if mod(iter,1) == 0
        ...
        LassoGlobal = shrinkage_gamma_seq;
        mu_w_tilde = mu(w_tilde_index); 
        b_w_tilde = b(w_tilde_index); 
        c_w_tilde = c(w_tilde_index);
        mean_w_tilde = mu_w_tilde'*mu_w_tilde+b_w_tilde'*b_w_tilde+sum(c_w_tilde.^2);
%         shrinkage_l2 = length(w_tilde_index)/mean_w_tilde;
    ...
    if(lbFlag)
        ...
        if (iter>LBwindow)
            ...
            lbGlobal = lb_bar;
            set(handles.txtPPSVal,'string',num2str(lb_bar(iter-LBwindow)));
...
%             disp(['Current LB: ',num2str(lb(iter))]);
%             if (iter>LBwindow)
%                 disp(['Current LB: ',num2str(lb_bar(iter-LBwindow))]);
%             end
